<article class="card" id="ajax_table">
    <div class="article-header">
        <?php echo lang('email-template-management'); ?>  
    </div>
    <div class="card-wrap">
        <div class="row site-controls-outer">
            ...
            <div class="col-sm-3">
                <a href="<?php echo site_url() . BASE_ADMIN_URL_CUSTOM . 'email_template/action/add' ?> " title="<?php lang('add_template') ?>" class="btn btn-blue pull-right"><span><img src="images/plus-icon.png" alt="Add Template"></span><?php echo lang('add_template'); ?></a>
            </div>
        </div>
        <!--       dedwe-->
        ...
        <div class="con">
            ...
            <div class="table-responsive"> 
                <?php if (!empty($email_template_model_list)) { ?>            
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="name-col">
                                    ...
                                    ?>
                                    <a href="javascript:void(0)" onclick="sort_data('c.template_name', '<?php echo $field_sort_order; ?>');" ><?php echo lang('template-name') ?></a>
                                    <?php
                        ...
                        <tbody>
                            ...
                            foreach ($email_template_model_list as $email_template_list) {
                                if ($i % 2 != 0) {
                                    $class = "odd-row";
                                } else {
                                    $class = "even-row";
                                ...
                                <tr class="<?php echo $class; ?> rows" id="row-<?php echo $email_template_list['c']['id']; ?>">
                                    <td>
                                        <div class="ckbox ckbox-default">
                                            <input type="checkbox" id="<?php echo $email_template_list['c']['id']; ?>" name="check_box[]" class="icheck check_box" value="<?php echo $email_template_list['c']['id']; ?>"/>
                                            <label for="<?php echo $email_template_list['c']['id']; ?>"></label>
                                        </div>
                                    </td>
                                    <td><?php echo $i; ?></td>
                                    <td><?php echo $email_template_list['c']['template_name']; ?></td>
                                    <td><?php
                                        if ($email_template_list['c']['status'] == 1) {
                                    ...
                                    <td>
                                        <a href="<?php echo site_url() . BASE_ADMIN_URL_CUSTOM ?>email_template/view_data/<?php echo $email_template_list['c']['id'] ?>" title="<?php echo lang('view'); ?>"><img src="images/search-icon-black.png" alt="<?php echo lang('view'); ?>"></a>
                                        <a href="<?php echo base_url() . BASE_ADMIN_URL_CUSTOM ?>email_template/action/edit/<?php echo $email_template_list['c']['id']; ?>" title="<?php echo lang('edit'); ?>"><img src="images/edit-icon.png" alt="<?php echo lang('edit'); ?>"></a>
                                        <?php
                                        $template_id = $email_template_list['c']['id'];
        ...
        $options = array(
            ...
            'isAjaxRequest' => 1,
            'base_url' => base_url() . BASE_ADMIN_URL_CUSTOM . "email_template/index",
            'params' => $querystr,
            'element' => 'ajax_table'
        );
...
$('#check_all').on('ifChecked', function(event) {
    ...
    function delete_records()
    {
        ...
        $.ajax({
            type: 'POST',
            url: '<?php echo base_url() . BASE_ADMIN_URL_CUSTOM ?>email_template/index',
            ...
            success: function(data) {
                ...
                ajaxLink('<?php echo base_url() . BASE_ADMIN_URL_CUSTOM ?>email_template/index', 'ajax_table', '<?php echo $querystr; ?>' + pageno);
                $("#messages").show();
                $("#messages").html(data);
                //unblockUI();
            }
    ...
    function delete_template(id) {
        ...
        if (res) {
            ...
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url() . BASE_ADMIN_URL_CUSTOM ?>email_template/delete',
