...
<article id="ajax_table" class="card actionplans-container">
    ...
    <div class="card-wrap">
        <div class="row">
            <form id="add-action-plan-form" method="post">
                <div class="col-sm-11">
                    <div class="row">
                        <div class="form-group col-sm-4">
                            <div class="form-dropdown">
                                <select name="action_category_id" data-type="custom-dropdown" id="action_category_id">
                                    <option value=""><?php echo lang('select-action-category'); ?></option>
                                    <?php
                                    if (!empty($action_categories)) {
                                        foreach ($action_categories as $acat) {
                                            ?>
                                            <option value="<?php echo $acat['pc']['id']; ?>"><?php echo $acat['pc']['name']; ?></option>
                                            <?php
                                        }
                                    }
                                    ?>
                        ...
                        <div class="form-group col-sm-4">
                            <div id="action_project_id_div" class="form-dropdown">
                                <select name="action_project_id" data-type="custom-dropdown" id="action_project_id">
                                    <option value=""><?php echo lang('select-action-project'); ?></option>
                                </select>
                            </div>
                        </div>
                        <div class="form-group col-sm-4">
                            <div id="action_todo_id_div" class="form-dropdown">
                                <select name="action_todo_id" data-type="custom-dropdown" id="action_todo_id">
                                    <option value=""><?php echo lang('select-action-todo'); ?></option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-sm-1">
                    <button id="addselectedactionplan" class="btn-control"><img alt="Plus" src="<?php echo base_url(); ?>themes/default/images/plus-icon.png"></button>
                </div>
            </form>        
        </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th class="text-center digits-col"><strong><?php echo 'Delete'; ?></strong></th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php
                            ...
                            if ($category['pc']['projects_todo_count'] > 0) {
                                ...
                                foreach ($category['pc']['projects'] as $pkey => $project) {
                                    foreach ($project['p']['project_todos'] as $key => $todo) {
                                        ...
                                        <tr>
                                            <?php if ($crow) { ?>
                                                <td rowspan="<?php echo $category['pc']['projects_todo_count']; ?>" class="category-title-td"><img src="<?php echo site_url() . 'themes/default/images/category/' . $category['pc']['category_static_image']; ?>"><strong><?php echo $project['pc']['name']; ?></strong></td>
                                            <?php } ?>
                                            ...
                                            <td class="col-sm-1"><input type="text" name="target_date" class="input-control datepicker datechange action-date-col" data-todo-id="<?php echo $todo['id']; ?>" value="<?php echo $targetdate; ?>" style="cursor:pointer !important;" /></td>
                                            ...
                                            <td class="col-sm-1"><input type="text" name="completed_date" class="input-control completeddatepicker action-date-col" data-todo-id="<?php echo $todo['id']; ?>" value="<?php echo $completeddate; ?>" /></td>
                                            <td class="col-sm-3">
                                                <div class="form-dropdown" >
                                            ...
                                            <td class="col-sm-1"><input onkeyup="this.value=addCommas(this.value);" type="text" id="kwh_savings" name="kwh_savings" class="input-control action-date-col kwh_savings"  data-todo-id="<?php echo $todo['id']; ?>" value="<?php echo $todo['kwh_savings']; ?>" /></td>
                                            <td class="col-sm-1"><div style="padding-right: 14px;margin-top: -18px;"><i style="color:#555;float: left; position: relative; left: 4px; top: 31px;font-size:13px;" aria-hidden="true"><?php echo CURRENCY_SYMBOL ?></i><input style="padding: 12px 3px;padding-left: 32px;" type="text" id="cost_savings" name="cost_savings" class="input-control action-date-col cost_savings" data-todo-id="<?php echo $todo['id']; ?>" value="<?php echo str_replace("$","", $todo['cost_savings']);?>" /></div></td>
                                            <td class="col-sm-2">
                                            ...
                                            <td>
                                                <a title="Delete" href="#" onclick="single_delete(<?php echo $todo['id']; ?>)"><img alt="Delete" src="images/delete-icon.png"></a>
                                            </td>
                                        </tr>
                                        <?php
                                        $crow = false;
    ...
    <div id="comment_form_container" class="card-wrap" style="display:none;">
        <form id="comment_form" class="site-info-form comment-upload-form" method="post">
    ...
    <div id="file_form_container" class="card-wrap" style="display:none;">
        <form id="file_form" enctype="multipart/form-data" class="site-info-form comment-upload-form" method="post" action="<?php echo site_url() . BASE_ADMIN_URL_CUSTOM . 'projects/add_project_todo_file' ?>">
            <ul class="form-outer-block">
                <li>
                    <label for="commentbox" class="main-label">Comment <span class="asterisk">*</span></label>
                    <div class="row">
...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            onSelect: function(selectedDate, obj) {
                ...
                $.ajax({
                    ...
                    data: {todo_id: todo_id, target_date: target_date}
                        ...
                        .done(function(data) {
                            ajaxLink('<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>projects/actionplans', 'ajax_table');
                            $("#messages").show();
                            $("#messages").html(data);
                        });
            }
        ...
        $(".completeddatepicker").datepicker({
            onSelect: function(selectedDate, obj) {
                var todo_id = $(this).attr('data-todo-id');
                var completed_date = selectedDate;

                $.ajax({
                    method: "POST",
                    url: "<?php echo site_url() . BASE_ADMIN_URL_CUSTOM . 'projects/addactionplan'; ?>",
                    data: {todo_id: todo_id, completed_date: completed_date}
                }).done(function(data) {
                    ajaxLink('<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>projects/actionplans', 'ajax_table');
                    $("#messages").show();
                    $("#messages").html(data);
        ...
        if (res) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    ajaxLink('<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>projects/actionplans', 'ajax_table', '<?php echo $querystr; ?>' + pageno);
                    $("#messages").show();
                    $("#messages").html(data);
                }
            });
