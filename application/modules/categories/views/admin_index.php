<article id="ajax_table" class="card">
    <div class="article-header"> Category Management </div>
    <div class="card-wrap menu-content-box">
        ...
        <!--                            <td><?php // echo $category_page['c']['slug_url']; ?></td>-->
                                    ...
                                    <td>
                                        ...
                                        {
                                            echo add_image(array('active.png'),'','',array('title' => 'Active'));
                                        }
                                        else
                                        {
                                            echo add_image(array('inactive.png'),'','',array('title' => 'Inactive'));
                                        }
                                        ?>
                                    </td>
                                    <td>
                                        
                                                <a title="View" href="<?php echo site_url () . BASE_ADMIN_URL_CUSTOM; ?>categories/view/<?php echo $category_page['l']['language_code'] . "/" . $category_page['c']['category_id']; ?>"><?php echo add_image (array ('search-icon-black.png')); ?></a>                              
                                                <a title="Edit" href="<?php echo site_url().BASE_ADMIN_URL_CUSTOM; ?>categories/action/edit/<?php echo $category_page['l']['language_code'] . "/" . $category_page['c']['category_id']; ?>" title="<?php echo lang('edit'); ?>"><?php echo add_image(array('edit-icon.png')); ?></a>
                                                <a title="Delete" href='javascript:;' title='<?php echo lang('delete'); ?>' onclick="delete_category('<?php echo $category_page['c']['id']; ?>', '<?php echo $category_page['c']['slug_url']; ?>')"><?php echo add_image(array('delete-icon.png')); ?></a>
                                    </td>
                                </tr>
                                <?php
                                $i++;
            ...
            if (!empty($category_list)){
                $options = array(
                    ...
                    'isAjaxRequest' => 1,
                    'base_url' => base_url().BASE_ADMIN_URL_CUSTOM."categories/index/" . $language_code,
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        openlink = function(type){
            ...
            location.href= "<?php echo base_url().BASE_ADMIN_URL_CUSTOM; ?>categories/action/add/";
        }
        ...
        delete_category = function(id,slug_url)
        {
            ...
            if(res){
               //blockUI();
                $.ajax({
                    type:'POST',
                    url:'<?php echo base_url().BASE_ADMIN_URL_CUSTOM; ?>categories/delete',
                    ...
                    success: function(data) {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: '<?php echo base_url().BASE_ADMIN_URL_CUSTOM; ?>categories/index',
    ...
    function active_records()
    {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                ajaxLink('<?php echo base_url().BASE_ADMIN_URL_CUSTOM; ?>categories/index','ajax_table','<?php echo $querystr; ?>'+pageno);
                $("#messages").show();
                $("#messages").html(data);
            }
        });
