                                    ...
                                    <tbody>
                                        ...
                                        foreach ($sites as $site) {
                                            if (!empty($reportdata[$site['id']]['electricity' . $base_type_variable])) {
                                                ...
                                                $currentdata = (!empty($reportdata[$site['id']]['electricity' . $base_type_variable]) && !empty($reportdata[$site['id']]['site_builtup_area'])) ? $reportdata[$site['id']]['electricity' . $base_type_variable] / $reportdata[$site['id']]['site_builtup_area'] : 0;
                                                $previousdata = (!empty($reportdata['previousdata'][$site['id']]['electricity' . $base_type_variable]) && !empty($reportdata['previousdata'][$site['id']]['site_builtup_area'])) ? $reportdata['previousdata'][$site['id']]['electricity' . $base_type_variable] / $reportdata['previousdata'][$site['id']]['site_builtup_area'] : 0;
...
<script type="text/javascript">
	...
	function drawHighchart() {
		...
		<?php foreach ($sites as $site) {
			if (!empty($reportdata[$site['id']]['electricity' . $base_type_variable])) {
				$kwh_per_built_area_data = (!empty($reportdata[$site['id']]['electricity' . $base_type_variable]) && !empty($reportdata[$site['id']]['site_builtup_area'])) ? ($reportdata[$site['id']]['electricity' . $base_type_variable] / $reportdata[$site['id']]['site_builtup_area']) : 0;
				$built_up_area_data = (!empty($reportdata[$site['id']]['site_builtup_area'])) ? $reportdata[$site['id']]['site_builtup_area'] : 0;

            $kwh_per_built_area_data = round($kwh_per_built_area_data, 2);
            $built_up_area_data = round($built_up_area_data, 2);
            ...
            $tooltip_data = $sitedata . ' : ' . $kwh_per_built_area_data;            
                ...
                if($currentdata > $previousdata) { ?>
					scatterDataArray.push(['<?php echo $sitedata; ?>', <?php echo $kwh_per_built_area_data; ?>, <?php echo $built_up_area_data; ?>, <?php echo $kwh_per_built_area_data; ?>, 'triangle', 'red']);
                <?php } else { ?>
					scatterDataArray.push(['<?php echo $sitedata; ?>', <?php echo $kwh_per_built_area_data; ?>, <?php echo $built_up_area_data; ?>, <?php echo $kwh_per_built_area_data; ?>, 'triangle-down', 'limegreen']);
		<?php }
			}
		}
		?>
		scatterSeries = [];
		Object.entries(scatterDataArray).forEach(([key, value]) => {
			scatterSeries.push({
				showInLegend: false,
				name: scatterDataArray[key][0],
				color: scatterDataArray[key][5],
				data: [
					[scatterDataArray[key][2], scatterDataArray[key][1]]
				],
				marker: {
					symbol: scatterDataArray[key][4],
				}
			}, );
		});
