                        ...
                        for ($j = 1; $j <= $totalCol; $j++) {
                            if (trim($dataCells[$i][$j]) != '') {
                                ...
                                if (!in_array($colmuns[trim($dataCells[0][$j])], $keyNotExists)) {
                                    $dataInsert[$colmuns[trim($dataCells[0][$j])]] = $dataCells[$i][$j];
                                }
                            } else {
                                continue;
                            }
                        ...
                        if ($totalRows > 1) {
                            ...
                            if (!empty($importData)) {
                                ...
                                foreach ($batchInsertData as $utilities) {
                                    if (isset($existingUtilities[$utilities['site_id']][$utilities['year_id']][$utilities['month_id']][$utilities['date_id']])) {
                                        $deleteId[] = $existingUtilities[$utilities['site_id']][$utilities['year_id']][$utilities['month_id']][$utilities['date_id']];
                                    }
                                }
                                // delete utilities if exists
                                if (!empty($deleteId)) {
                                    $this->import_model->deleteDailyUtilityIfexists($deleteId);
                                }
                                // Insert new data
                        ...
                        if ($totalRows > 1) {
                            ...
                            if (!empty($allData)) {
                                ...
                                if (!empty($dynamicDeleteIds) && $statusInsertDynamicUtility == true) {
                                    $existingFixedData = $this->import_model->deleteDailyDynamicSubmissionUtilityIfexists($dynamicDeleteIds);
                                }
                                //Dynamic data delete and insert
                                ...
                                if (!empty($fixedDelete) && $statusInsertFixedUtility == true) {
                                    $this->import_model->deleteDailyFixedSubmissionUtilityIfexists($fixedDelete);
                                }
                                //Fixed data delete and insert                              
                            }
                        }
