...
class Reportscron_admin extends Base_Admin_Controller
{
    ...
    private function access_rules()
    {
        return array(
            array(
                'actions' => array('index', 'upper_management_report', 'get_image_from_uri', 'generate_mpdf', 'send_mail_to_user'),
                'users'   => array('*'),
            ),
        );
    }
    ...
    public function generate_mpdf()
    {
        ...
        // loop to get sites data in table format for PDF
        foreach ($data['sites'] as $key => $site) {
            $data['site_key'] = $key;
            $data['temp']     = $temp;
            $postData         = $data;
        ...
        $this->mailer->mail->AddAddress($user['email']); 
        //$this->mailer->mail->AddAddress('rahul.patel@tatvasoft.com'); 
        $this->mailer->mail->Subject = $subject;
        $this->mailer->mail->Body    = $body;
