<script type="text/javascript" src="<?php echo site_url(); ?>themes/default/js/jquery.min.js"></script>
...
?>        
    ...
    <?php   
        ...
        var options = {
            ...
            hAxis: {title: '<?php echo lang("sites"); ?>', titleTextStyle: {fontName: 'Arial', fontSize: 18}, slantedText: true, slantedTextAngle: 90},
            vAxes: {
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
            // setTimeout(function(){
                ...
                $.ajax({
                    ...
                    data: {<?php echo $csrf_token; ?>: '<?php echo $csrf_hash; ?>',img_url: $("#sites_chart_cost_img").val(), filenm: 'sites_chart_cost', user_id: "<?php echo $user['id'];?>"},
                    error: function() {
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
            // setTimeout(function(){
                ...
                $.ajax({
                    ...
                    data: {<?php echo $csrf_token; ?>: '<?php echo $csrf_hash; ?>',img_url: $("#sites_chart_img").val(), filenm: 'sites_chart', user_id: "<?php echo $user['id'];?>" },
                    error: function() {
                        // alert("Server problem. Please try again.");
                        return false;
                    },
    ...
    if (!empty($sites)) { ?>
        ...
        if (!empty($sites)) {
            ...
            foreach ($sites as $site) {
                ...
                $data_electricity        = (!empty($utilityArray[$site['id']]['total_electricity_kwh'])) ? $utilityArray[$site['id']]['total_electricity_kwh'] : 0;
                ...
                // pre($chart_data['data']['utility_cost_chart']);
                // pr($totalElectricity);
                $array_1[] = array('site' => $sitedata,'data_electricity' => $totalElectricity, 'data_fuel' => $totalFuel, 'data_lpg' => $totalLpg, 'data_natural_gas' => $totalNaturalGas, 'data_water' => $totalWater, 'data_heating_district' => $totalHeatingDistrict, 'data_cooling_district' => $totalCoolingDistrict, 'data_occupancy' => $total_sum_data_occupancy);
        ...
        ?>
            ...
            var arrValues = ['<?php echo $val['site']; ?>'];
                    <?php
                     if($totalElectricity != 0){ ?>
                        arrValues.push(<?php echo $val['data_electricity']; ?>);
                    <?php   } ?>
                    <?php if($totalFuel != 0){ ?>
                                arrValues.push(<?php echo $val['data_fuel']; ?>);
                    <?php   } ?>
                    <?php if($totalLpg != 0){ ?>
                                arrValues.push(<?php echo $val['data_lpg']; ?>);
                    <?php   } ?>
                    <?php if($totalNaturalGas != 0){ ?>
                                arrValues.push(<?php echo $val['data_natural_gas']; ?>);
                    <?php   } ?>
                    <?php if($totalWater != 0){ ?>
                                arrValues.push(<?php echo $val['data_water']; ?>);
                    <?php   } ?>
                    <?php if($totalHeatingDistrict != 0){ ?>
                                arrValues.push(<?php echo $val['data_heating_district']; ?>);
                    <?php   } ?>
                    <?php if($totalCoolingDistrict != 0){ ?>
                                arrValues.push(<?php echo $val['data_cooling_district']; ?>);
                    <?php   } ?>
                    
                arrValues.push(<?php echo $val['data_occupancy']; ?>);
                arrValuesMulti.push(arrValues);
        <?php   
        }
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
            ...
            $.ajax({
                ...
                async: false,
                url: '<?php echo base_url() . $this->_data["section_name"]; ?>reportscron/get_image_from_uri_new',
                data: {<?php echo $csrf_token; ?>: '<?php echo $csrf_hash; ?>',img_url: $("#sites_chart_build_area_img_carbon").val(), filenm: 'utility_cost_chart_carbon_footprint', user_id: "<?php echo $user['id'];?>" },
                error: function() {
                    // alert("Server problem. Please try again."); 
                    return false;
                },
    ...
    <?php   
        ...
        google.visualization.events.addListener(chart, 'ready', function () {
            ...
            $.ajax({
                ...
                async: false,
                url: '<?php echo base_url() . $this->_data["section_name"]; ?>reportscron/get_image_from_uri',
                data: {<?php echo $csrf_token; ?>: '<?php echo $csrf_hash; ?>',img_url: $("#sites_chart_build_area_img").val(), filenm: 'sites_chart_build_area', user_id: "<?php echo $user['id'];?>" },
                error: function() {
                    // alert("Server problem. Please try again."); 
                    return false;
                },
