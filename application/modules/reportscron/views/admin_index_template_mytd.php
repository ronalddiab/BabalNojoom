...
$id = $site_detail['id'];
$baseline_regression_year = !empty($site_detail['baseline_regression_year']) ? $site_detail['baseline_regression_year'] : (date('Y') - 1);
...
<script type="text/javascript">
	...
	var _reportLastDrawChartStart = 0;

	var _reportResizeDrawTimer = null;

	function drawChart() {

		_reportLastDrawChartStart = Date.now();

		/* REGRESSION CHART AT END APPEND */
		<?php foreach ($monthly['regression']['regressionUtility'] as $key => $utilityLabel) { ?>
			var responseParsed<?php echo $key; ?> = <?php echo json_encode($monthly['regression'][$utilityLabel]);?>;
			if (responseParsed<?php echo $key; ?>) {
				var data_cur<?php echo $key; ?> = new google.visualization.arrayToDataTable(responseParsed<?php echo $key; ?>);
				var options_cur<?php echo $key; ?> = {
					height: 700,
					isStacked: false,
					title: '<?php echo $monthly['regression']['utility_array'][$utilityLabel]['Label'] . " - " . "Actual vs Prediction" . " - " . ($filters['report_year']); ?>',
					titleTextStyle: {
						fontName: 'Arial',
						fontSize: 20
					},
					...
					vAxes: {
						0: {
							title: '<?php echo $monthly['regression']['utility_array'][$utilityLabel]['Label'] . " (" . GetSiteUtilityUnitName($site_id, $utilityLabel) . ")" ?>',
							titleTextStyle: {
								fontName: 'Arial',
								fontSize: 18
							},
				...
				var current_year_energy_chart<?php echo $key; ?> = new google.visualization.ColumnChart(document.getElementById('utility_regression_<?php echo $utilityLabel; ?>_div_<?php echo $id; ?>'));
				google.visualization.events.addListener(current_year_energy_chart<?php echo $key; ?>, 'ready', function() {
					setTimeout(function() {
						...
						document.getElementById('utility_regression_<?php echo $utilityLabel; ?>_Img_<?php echo $id; ?>').value = imgUri<?php echo $key; ?>;
					}, 1000);
				});
				current_year_energy_chart<?php echo $key; ?>.draw(data_cur<?php echo $key; ?>, options_cur<?php echo $key; ?>);
			}

			var responseParsedLY<?php echo $key; ?> = <?php echo json_encode($monthly['regression'][$utilityLabel.'_LY']); ?>;
			if (responseParsedLY<?php echo $key; ?>) {
				...
				var options_LY_<?php echo $key; ?> = {
					height: 700,
					title: '<?php echo $monthly['regression']['utility_array'][$utilityLabel]['Label'] ." - " . "Regression Model" . " - " . ($baseline_regression_year); ?>',
					titleTextStyle: {
						fontName: 'Arial',
						fontSize: 20
					},
				...
				google.visualization.events.addListener(LY_energy_chart<?php echo $key; ?>, 'ready', function() {
					setTimeout(function() {
						...
						document.getElementById('utility_regression_LY_<?php echo $utilityLabel; ?>_Img_<?php echo $id; ?>').value = imgUri_LY_<?php echo $key; ?>;
					}, 1000);
				});
      ...
      $colorCoolingDistrict = ($totalCoolingDistrict != 0) ? $chart_legend_colors['District_Cooling'] : ''; */
		...
		if (!empty($utility_cost_chart)) {
			...
			google.visualization.events.addListener(carbonchart, 'ready', function () {
				...
				var imgUri = carbonchart.getImageURI();

				// set hidden input
				$('#columnChartCarbonFootprintImg_<?php echo $id; ?>').val(imgUri);
	...
	$(window).on('resize', function () {
		...
		clearTimeout(_reportResizeDrawTimer);

		_reportResizeDrawTimer = setTimeout(function () {

			if (Date.now() - _reportLastDrawChartStart < 2500) {
		...
		<div class="row">
			...
			<?php foreach ($monthly['regression']['regressionUtility'] as $key => $utilityLabel) { ?>
				<div class="col-sm-12">
					<div class="panel panel-primary">
						<div class="panel-body">
							<div id="utility_regression_LY_<?php echo $utilityLabel; ?>_div"></div>
							<div id="utility_regression_<?php echo $utilityLabel; ?>_div_<?php echo $id; ?>"></div>
						</div>
					</div>
				</div>
			<?php } ?>
