...
class Reportscron_admin extends Base_Admin_Controller
{
    ...
    private function access_rules()
    {
        return array(
            array(
                'actions' => array('index', 'upper_management_report', 'get_image_from_uri', 'generate_mpdf', 'send_mail_to_user','prepare_chart'),
                'users' => array('*'),
            ),
        );
    }
    ...
    public function index($cview = 'index')
    {
        ...
        // pr(REPORT_CURRENCY.'   '.REPORT_CURRENCY_SYMBOL);
        // pr("REPORT_CURRENCY : ".REPORT_CURRENCY);
        // pr("REPORT_CURRENCY_SYMBOL : ".REPORT_CURRENCY_SYMBOL);
        // pr("CURRENCY : ".CURRENCY);
        // pre("CURRENCY_SYMBOL : ".CURRENCY_SYMBOL);
        ...
        $to = "dipen.suthar@tatvasoft.com";
        $subject = "HEP cron - before chart";
        $txt = "Hello world!";
    ...
    public function get_image_from_uri($url = '')
    {
        ...
        $imageName = uniqid() . uniqid() . uniqid() . '_' . time() . '.png';
        $imagePath = BASE_PATH_CUSTOM . "/assets/uploads/cron/" . $imageName;
        $imageUrl = site_url() . "assets/uploads/cron/" . $imageName;
    ...
    public function getMonthlyReportChart($month, $year)
    {
        ...
        if (!empty($utility_cost_chart_results)) {
            foreach ($utility_cost_chart_results as $key => $value) {
                ...
                $utility_cost_chart['utility_cost_chart_title'] = lang('utility-cost-chart-title') . ' (' . $cur_symbol . ')';
            ...
            foreach ($resultkeysMonthlyreport as $year => $value) {
                foreach ($value as $key1 => $month) {
                    ...
                    if ($pre_data_electricity != 0 || $data_electricity != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_electricity * $site_details['electricity_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_electricity * $site_details['electricity_emission_factor'], $decimal_places);
                    ...
                    if ($pre_data_fuel != 0 || $data_fuel != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_fuel * $site_details['fuel_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_fuel * $site_details['fuel_emission_factor'], $decimal_places);
                    ...
                    if ($pre_data_lpg != 0 || $data_lpg != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_lpg * $site_details['lpg_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_lpg * $site_details['lpg_emission_factor'], $decimal_places);
                    ...
                    if ($pre_data_natural_gas != 0 || $data_natural_gas != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_natural_gas * $site_details['natural_gas_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_natural_gas * $site_details['natural_gas_emission_factor'], $decimal_places);
                    ...
                    if ($pre_data_heating_district != 0 || $data_heating_district != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_heating_district * $site_details['district_heating_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_heating_district * $site_details['district_heating_emission_factor'], $decimal_places);
                    ...
                    if ($pre_data_cooling_district != 0 || $data_cooling_district != 0) {
                        ...
                        $carbon_footprint[1][] = round($pre_data_cooling_district * $site_details['district_cooling_emission_factor'], $decimal_places);
                        $carbon_footprint[2][] = round($data_cooling_district * $site_details['district_cooling_emission_factor'], $decimal_places);
                ...
                if (!empty($site_ids)) {
                    ...
                    if (!empty($data['sites'])) {
                        ...
                        foreach ($data['sites'] as $key => $site_detials) {
                           ...
                           /* $data['sites'][$key]['data']['kpi']['total_utility_cost_lastMonth'] = $getUtilities_lastMonth['total_electricity_cost'] + $getUtilities_lastMonth['total_fuel_oil_cost'] + $getUtilities_lastMonth['total_lpg_cost'] + $getUtilities_lastMonth['total_natural_gas_cost'] + $getUtilities_lastMonth['district_heating_cost'] + $getUtilities_lastMonth['district_cooling_cost'] + $getUtilities_lastMonth['water_total_consumption_cost'];
...
*/
                            ...
                            /*$data['sites'][$key]['data']['kpi']['total_utility_cost_sameMonth_lastYear'] = $getUtilities_sameMonth_lastYear['total_electricity_cost'] + $getUtilities_sameMonth_lastYear['total_fuel_oil_cost'] + $getUtilities_sameMonth_lastYear['total_lpg_cost'] + $getUtilities_sameMonth_lastYear['total_natural_gas_cost'] + $getUtilities_sameMonth_lastYear['district_heating_cost'] + $getUtilities_sameMonth_lastYear['district_cooling_cost'] + $getUtilities_sameMonth_lastYear['water_total_consumption_cost'];
                            ...
                            if (!empty($utility_cost_chart_results)) {
                                ...
                                //Add total values in data array
                                $data['sites'][$key]['totalElectricity'] = $totalElectricity;
                                $data['sites'][$key]['totalFuel'] = $totalFuel;
                                $data['sites'][$key]['totalLpg'] = $totalLpg;
                                $data['sites'][$key]['totalNaturalGas'] = $totalNaturalGas;
                                $data['sites'][$key]['totalWater'] = $totalWater;
                                $data['sites'][$key]['totalHeatingDistrict'] = $totalHeatingDistrict;
                                $data['sites'][$key]['totalCoolingDistrict'] = $totalCoolingDistrict;
                                $data['sites'][$key]['utility_year_selected'] = $utility_year_selected;

                                $data['sites'][$key]['totalElectricityConsumption'] = $totalElectricityConsumption;
                                $data['sites'][$key]['totalFuelConsumption'] = $totalFuelConsumption;
                                $data['sites'][$key]['totalLpgConsumption'] = $totalLpgConsumption;
                                $data['sites'][$key]['totalNaturalGasConsumption'] = $totalNaturalGasConsumption;
                                $data['sites'][$key]['totalWaterConsumption'] = $totalWaterConsumption;
                                $data['sites'][$key]['totalHeatingDistrictConsumption'] = $totalHeatingDistrictConsumption;
                                $data['sites'][$key]['totalCoolingDistrictConsumption'] = $totalCoolingDistrictConsumption;
                            } else {
                                $data['utility_cost_chart'] = array();
                            }
    ...
    public function generate_mpdf()
    {
        ...
        // $this->mailer->mail->AddAddress($user['email']);
        // // $this->mailer->mail->AddAddress('rdiab@eegroup.info'); 
        // $this->mailer->mail->AddAddress('surbhi.ladhava@pokemail.net');
        // $this->mailer->mail->AddAddress('mrugesh.kshatriya@tatvasoft.com');
        $this->mailer->mail->AddAddress('surbhi.ladhava@tatvasoft.com');
        $this->mailer->mail->Subject = $subject;
        $this->mailer->mail->Body = $body;
        exit;
