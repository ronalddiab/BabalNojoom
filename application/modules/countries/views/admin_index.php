<article class="card" id="ajax_table">
    <div class="article-header">
        <?php echo lang('country-management'); ?>
    </div>
    <div class="card-wrap">
        <div class="row country-controls-outer">            
                ...
                <form class="default-form form-inline clearfix">
                    <div class="form-group form-control-outer">
                        ...
                        $input_data = array(
                            ...
                            'class' => 'form-control',
                            //'title' => 'search',
                            'placeholder' => lang('search-by-country'),
            ...
            <?php $add_country_permission = check_user_permission_by_label('admin.countries.edit'); ?>
            <?php if($add_country_permission){ ?>
            <div class="col-sm-2 col-lg-3">
                <?php echo anchor(BASE_ADMIN_URL_CUSTOM . 'countries/edit', '<span>' . add_image(array('plus-icon.png')) . '</span> ' . lang('add-country'), 'class="btn btn-blue pull-right"'); ?>
            </div>
            <?php } ?>
        </div>
                ...
                <?php if (!empty($countries)) { ?>
                    <thead>
                        <tr>
                            ...
                            <th class="name-col">
                                ...
                                if ($sort_by == 'c.country' && $sort_order == 'asc') {
                                    $sort_image = 'srt_up.png';
                                    $field_sort_order = 'desc';
                                }
                                ?>
                                <a href="javascript:void(0)" onclick="sort_data('c.country', '<?php echo $field_sort_order; ?>');" >
                                    <?php echo lang('country'); ?>
                                    <?php
                                    if ($sort_by == 'c.country') {
                                        ?>
                                        <div class="sorting">
                    ...
                    <tbody>
                        ...
                        foreach ($countries as $country) {
                            $country_id = $country['c']['id'];
                            ?>
                            <tr>
                                <td>
                                    <input type="checkbox" id="<?php echo $country['c']['id']; ?>" name="check_box[]" class="icheck checkboxlist check_box" value="<?php echo $country['c']['id']; ?>">
                                </td>
                                <td><?php echo $i; ?></td>
                                <td><?php echo $country['c']['country']; ?></td>
                                <td>
                                    <?php
                                    if ($country['c']['status'] == 1) {
                                ...
                                <td>
                                    <a title="View" href="<?php echo site_url() . BASE_ADMIN_URL_CUSTOM; ?>countries/view_data/<?php echo $country_id; ?>"><?php echo add_image(array('search-icon-black.png')); ?></a>
                                    <?php $edit_country_permission = check_user_permission_by_label('admin.countries.edit'); ?>
                                    <?php if($edit_country_permission){ ?>
                                    <a href="<?php echo site_url() . BASE_ADMIN_URL_CUSTOM; ?>countries/edit/<?php echo $country_id ?>" title="<?php echo lang('edit') ?>"><?php echo add_image(array('edit-icon.png')); ?></a></div>
                                    <?php } ?>
                                    <?php
                                    $encrypted_id = base64_encode($country_id);

                                    $deletelink = "<a href='javascript:;' title='Delete' onclick='delete_country(\"$encrypted_id\")'>" . add_image(array('delete-icon.png')) . "</a>";
                                    ?>
                                    <?php echo $deletelink ?>
                                </td>
                            </tr>
        ...
        $options = array(
            ...
            'isAjaxRequest' => 1,
            'base_url' => base_url() . BASE_ADMIN_URL_CUSTOM . "countries/index",
            'params' => $querystr,
            'element' => 'ajax_table'
        );
...
<script type="text/javascript">
    ...
    function delete_records()
    {
        ...
        if (res) {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>countries/index',
...
//                    alert(data); return;
                    ...
                    ajaxLink('<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>countries/index', 'ajax_table', '<?php echo $querystr; ?>' + pageno);
                    $("#messages").show();
                    $("#messages").html(data);
                }
            });
    ...
    function delete_country(id) {
        ...
        if (res) {
            $.ajax({
                type: 'POST',
                url: '<?php echo base_url() . BASE_ADMIN_URL_CUSTOM; ?>countries/delete',
